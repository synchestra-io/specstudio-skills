...
AND the user invokes `spec-studio:specify` against that Idea
...
GIVEN the user asks `spec-studio:specify` to manually edit the source Idea's `promotes_to` or `Status` field directly
